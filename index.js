...
$(document).ready(function () {
  ...
  for (let i = 0; i < playlistData.length; i++) {
    createCard(playlistData[i]);
  }

  function createCard(data) {
    var div = $("<div>").addClass("card").attr("id", data.id);
    var img = $("<img>").addClass("card-img").attr("src", data.thumbnail);
    var heading = $("<h3>").text(data.title);
    div.append(img, heading);
    $("#playlist-wrapper").append(div);
    ...
    div.on("click", function () {
      ...
      $("#" + data.id).addClass("active-card");
      videoCard(data.id);
    });
  }

  function videoCard(id) {
    var selectedVideo = id - 1;
    $("#video-player").attr(
      "src",
      `https://player.vimeo.com/video/${videoPlaySectionData[selectedVideo].vimeoId}`
    );
    $("#video-title").text(videoPlaySectionData[selectedVideo].title);
    $("#video-description").text(
      videoPlaySectionData[selectedVideo].description
    );
    $("#views-count").text(videoPlaySectionData[selectedVideo].views);
    window.scrollTo(0, 0);
  }
